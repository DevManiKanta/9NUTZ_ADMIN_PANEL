...
//       });
//       const json = await res.json().catch(() => null);
//       // possible shapes: { status:true, data: [...] } OR array directly
//       let list: any[] = [];
//       if (Array.isArray(json)) list = json;
//       else if (Array.isArray(json?.data)) list = json.data;
//       else {
...
//       // Determine created category shape
...
//       // Fallback heuristics: if API returns the record inside 'data' as array or object
...
//       // refresh categories and auto-select the new one (if possible)
//       await fetchCategories();
//       setCatModalOpen(false);
//       toast.success("Category created");

//       // set newly created in the expense form if found
//       if (createdId != null) {
//         setForm((f) => ({ ...f, category_id: String(createdId) }));
//       } else if (createdName) {
//         // if id not returned but name returned, try to match by name in categories
...
//                       // ensure the newly created category is selected in the expense form
