...
//         return;
//       }
//       payload.tracking_id = shipmentNote.trim();
//     }

//     const token = localStorage.getItem("token");
//     const res = await api.post("/update-shipment-status", payload, {
//       headers: {
...
//     // Use backend's own message if available
//     if (res.data?.status === true) {
//       toast.success(res.data?.message || "Shipment updated successfully");
//     } else {
//       toast.error(res.data?.message || "Failed to update shipment");
//     }
...
//         {/* Tracking ID input only if Shipping selected */}
//         {shipmentStatus === "Shipping" && (
//           <label className="block text-sm">
//             <div className="text-xs text-gray-600 mb-1">Tracking ID</div>
//             <input
//               type="text"
//               value={shipmentNote}
...
//           <button
//   onClick={async () => {
//     // Wait for shipment API call to finish first
//     await handleShipmentSubmit();
...
export default function QrPayments() {
  ...
  const [shipmentOpen, setShipmentOpen] = useState(false);
  const [shipmentNote, setShipmentNote] = useState(""); // will map to awb_number
  const [shipmentUrl, setShipmentUrl] = useState(""); // shipping_url
  const [shipmentStatus, setShipmentStatus] = useState("Order Confirmed");

  // dedicated order used only for shipment modal to avoid opening details modal
  const [shipmentOrder, setShipmentOrder] = useState<SaleRow | null>(null);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3 sm:p-4 md:p-6">
      ...
      <div className="w-full mx-auto">
        <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1"></div>
            <div className="mb-4">
              <div className="inline-flex w-full sm:w-auto rounded-lg border bg-white overflow-hidden">
                <button
                  onClick={() => setActiveTab("online")}
                      ...
                      <td className="px-4 py-3 text-sm">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              // use dedicated shipmentOrder so we DON'T open details modal (selected)
                              setShipmentOrder(r);
                              setShipmentStatus("Shipping"); // optional default
                              setShipmentNote("");
                              setShipmentUrl("");
                              setShipmentOpen(true);
                            }}
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-t flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="text-xs sm:text-sm text-gray-600">
              Total Amount:{" "}
              <span className="font-semibold">₹{formatCurrency(rows.reduce((acc, r) => acc + (Number(r.amount) || 0), 0))}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Shipment / Update Order Status Modal */}
      {shipmentOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md p-4 z-10">
            ...
            <div className="space-y-4">
              ...
              {/* Tracking ID & Shipping URL only if Shipping selected */}
              {shipmentStatus === "Shipping" && (
                <>
                  <label className="block text-sm">
